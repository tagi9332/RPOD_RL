...
# ... (Keep previous relative state logic) ...
    ...
    # Generate random Axis-Angle for the Body-to-Hill (BH) offset
    axis = random_unit_vector()
    angle = np.random.uniform(0, 2 * np.pi)
    
    # Construct skew-symmetric matrix for the axis
    K = np.array([
        [0, -axis[2], axis[1]],
        [axis[2], 0, -axis[0]],
        [-axis[1], axis[0], 0]
    ...
    # Rodrigues' rotation formula to get the DCM
    BH = np.eye(3) + np.sin(angle) * K + (1 - np.cos(angle)) * (K @ K)
    
    a = chief_orbit.a
    T = np.sqrt(a**3 / mu) * 2 * np.pi # type: ignore
    
    # Orbital rate mapped into the Inertial (N) frame
    omega_HN_N = z * 2 * np.pi / T 
    
    args[rso]["sigma_init"] = C2MRP(BH @ HN)
    
    # Transform orbital rate into the newly randomized Body frame
    args[rso]["omega_init"] = BH @ HN @ omega_HN_N 
